...
|---|---|---|---|
| 20 Aug 2026 | Flags + lens | 🚩 **The flag collapse never worked, and the class it set was always correct.** `grid-template-rows:0fr` zeroes the FIRST grid row; the flags were direct children, so with three flags rows two and three stayed `auto` and a "closed" stack was still **74px tall** — dead space above the food table. Rebuilt as an **absolute overlay**: out of flow entirely, arrives one flag at a time (150ms steps), shrinks into the badge with a computed `transform-origin`. ⚠ Scoping that CSS to `#flags` revealed a second bug — the **save-risk modal's flags have been invisible since 18 Aug**, inheriting `opacity:0` from the shared `.fl` class. ⚠ And the first overlay build painted over the metric tiles: an absolutely positioned **flex** child's static position is the flex container's corner, not its flow position — fixed with a zero-height anchor. Chart chip **"plan change" → P꜀**, drawn glyph by glyph. **NEW: whole roast chicken** by cooked weight (223/24/0/13.4 skin-on, 167/25/0/6.6 stripped). **NEW: hover hint** — where eating one leaves you, the inverse of the close-the-day amount. **NEW: quality lens** — borders by protein per 100 kcal, off by default, with a 120 kcal floor so small food is never called bad. Suites: verify 35/35 · smoke **116 → 143** · steptest 17/17 | Confirm the 18 Aug composition · `health-fitness.md` split |
...
- 🚩 **`grid-template-rows: 0fr` collapses ONE row.** The 0fr→1fr height-animation idiom
  needs a single wrapper child; with N children only the first collapses and the rest sit
  in implicit `auto` tracks. It failed silently and looked like a partially-working
  animation. **And the test could not see it, because it asserted the CLASS was removed —
  which it always was.** Replaced with a geometric check: the food table's top must not
  move between flags-up and flags-down. **Assert the pixel, not the class name.**
- **An absolutely positioned FLEX child does not keep its flow position.** The spec puts
  its static position at the flex container's content-box corner, so the overlay landed on
  top of the metric tiles. A zero-height in-flow anchor is the fix. Caught on the live site
  after the suite went green, which is the third time this month that "tests pass" and
  "looks right" have come apart.
- **A density-only quality lens libels small food.** 68-kcal frozen veg has the same
  protein-per-calorie as cheddar and got the same amber border, which is nonsense — it
  cannot damage a day at any ratio. The floor is asymmetric on purpose: **small items can
  still earn green, they just cannot be called bad.**
